  ...
  const onSubmit = async () => {
    try {
      ...
      const payload: SignupPayload = {
        ...
        email: normalizeEmail(form.email), // ✅ メールアドレスを正規化
      };
      ...
      // 3) patients配下が参照するキー名���保存
      localStorage.setItem("access_token", token);

      // ✅ レスポンスから取得（フォールバック: JWTからデコード）
      const clinicId = loginJson.clinic_id ?? decoded?.clinic_id;
      const clinicName = loginJson.clinic_name ?? decoded?.clinic_name;

      if (clinicId) {
        localStorage.setItem("clinic_id", String(clinicId));
      }
      if (clinicName) {
