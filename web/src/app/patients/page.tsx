...
//   // ✅ フィルタ状態を追加
//   const [schoolFilter, setSchoolFilter] = useState<string>(""); // 学校名フィルタ
//   const [sexFilter, setSexFilter] = useState<string>("");       // 性別フィルタ（"", "male", "female"）
...
//     // 学校名フィルタ
//     if (schoolFilter) {
//       result = result.filter((p) => p.school_name === schoolFilter);
//     }
...
//     return result;
//   }, [patients, q, schoolFilter, sexFilter]);
...
//                   onChange={(e) => setQ(e.target.value)}
//                   placeholder="氏名 / 学校 / 生年月日で検索"
//                   className="h-11 w-full rounded-xl border border-slate-300 px-4 text-sm focus:border-[#173b7a] focus:outline-none"
//                 />
//                 {q ? (
//                   <button
//                     onClick={() => setQ("")}
...
//                   {patients.length === 0 ? "受検者がいません" : "該当する受検者がいません"}
//                 </div>
//                 <div className="mt-2 text-sm text-slate-600">
//                   {patients.length === 0 ? (
//                     "「受検者を新規登録」から1人目を追加してください。"
...
//                       <button
//                         onClick={resetFilters}
//                         className="mx-1 font-bold text-[#173b7a] underline hover:text-[#2a61c9]"
//                       >
//                         フィルタをリセット
//                       </button>
//                       してください。
...
//                   <div className="border-t border-slate-100 px-5 py-4 text-xs text-slate-500">
//                     ※ 検索・フィルタはフロント側で処理しています（API検索は必要になったら追加）
//                   </div>
//                 </div>
//               </div>
//             )}
...
export default function PatientsPage() {
  ...
  const [measuredDateFilter, setMeasuredDateFilter] = useState<string>(""); // ✅ 追加
  ...
  useEffect(() => {
    ...
    const fetchPatients = async () => {
      ...
      try {
        ...
        if (measuredDateFilter) {
          params.append("measured_date", measuredDateFilter);
        }
        const queryString = params.toString() ? `?${params.toString()}` : "";

        const res = await fetch(`${base}/patients${queryString}`, {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
    ...
    fetchPatients();
  }, [router, measuredDateFilter]); // ✅ 依存配列に追加
          ...
          <div className="border-b px-6 py-5">
            ...
            <div className="mt-6 grid grid-cols-1 gap-3 md:grid-cols-4">
              ...
              <div>
                <input
                  type="date"
                  value={measuredDateFilter}
                  onChange={(e) => setMeasuredDateFilter(e.target.value)}
                  className="h-11 w-full rounded-xl border border-slate-300 px-4 text-sm focus:border-[#173b7a] focus:outline-none"
                />
              </div>
            </div>

            {(q || schoolFilter || sexFilter || measuredDateFilter) && (
              <div className="mt-3 flex justify-end">
                <button
                  onClick={resetFilters}
                  className="rounded-lg bg-slate-100 px-4 py-2 text-xs font-bold text-slate-700 hover:bg-slate-200"
            ...
            {loading ? (
              ...
              <div className="overflow-hidden rounded-2xl border border-slate-200 shadow-sm">
                <div className="bg-gradient-to-r from-slate-900 to-slate-700 px-5 py-3">
                  <div className="flex items-center justify-between">
                    <div className="text-sm font-semibold text-white">
                      受検者一覧（{filtered.length}名
                      {measuredDateFilter && ` / ${measuredDateFilter}に測定`}）
                    </div>
                    <div className="text-xs text-slate-200">
                      ※ clinic_id はJWTで判定（URL/画面からは渡しません）
                    </div>
                ...
                <div className="overflow-x-auto bg-white">
                  ...
                  <div className="border-t border-slate-100 px-5 py-4 text-xs text-slate-500">
                    ※ 検索・フィルタはフロント側で処理（測定日フィルタはAPI検索）
                  </div>
                </div>
              </div>
            )}
