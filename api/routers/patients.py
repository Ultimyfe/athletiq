...
@router.get("/patients")
def list_patients(
    measured_date: Optional[str] = Query(None),  # ✅ 追加: YYYY-MM-DD形式
    clinic=Depends(get_current_clinic),
    db: Session = Depends(get_db),
):
    clinic_id = clinic["clinic_id"]
    ...
    if measured_date:
        rows = db.execute(
            text("""
                SELECT DISTINCT p.*
                FROM patients p
                INNER JOIN diagnose_records dr
                  ON dr.patient_id = p.id
                  AND dr.clinic_id = p.clinic_id
                WHERE p.clinic_id = :clinic_id
                  AND dr.measured_at = CAST(:measured_date AS date)
                ORDER BY p.id DESC
            """),
            {"clinic_id": clinic_id, "measured_date": measured_date},
        ).mappings().all()
    else:
        # 既存ロジック（全件取得）
        rows = db.execute(
